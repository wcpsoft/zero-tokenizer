#!/usr/bin/env python3
"""
BPE (Byte Pair Encoding) 分词器训练示例

本示例展示了如何使用文本数据训练一个新的BPE分词器。
"""
...
def generate_sample_text():
    ...
    english_sentences = [
        ...
        "BPE algorithm is a commonly used subword tokenization method.",
...
def main():
    ...
    try:
        ...
        # 测试训练后的分词器
        print("\n4. 测试训练后的分词器...")
        test_text = "这是一个测试BPE分词器的句子。This is a test sentence."
        ...
        # 保存训练好的分词器
        model_file = "bpe_tokenizer.json"
        print(f"\n6. 保存训练好的分词器到 {model_file}...")
        tokenizer.save(model_file)
        print("✓ 分词器保存成功")
        
        # 加载保存的分词器
        print("\n7. 加载保存的分词器...")
        loaded_tokenizer = zero_tokenizer.bpe()
